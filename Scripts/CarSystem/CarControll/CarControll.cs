...
public class CarControll : MonoBehaviour
{
    ...
    [SerializeField] private VehicleTuningProfile tuningProfile;
    [SerializeField] private bool applyTuningProfileOnStart = true;

    [Header("Upgrade State")]
    [SerializeField] private VehicleUpgradeState upgradeState = new VehicleUpgradeState();
    [SerializeField] private bool applyUpgradeStateOnStart = true;

    private VehicleTuningProfile runtimeTuningProfile;
    ...
    public void ApplyTuningProfile(VehicleTuningProfile profile)
    {
        if (profile == null)
            return;
        ...
        reverseAcceleration = profile.reverseAcceleration;
        brakeForce = profile.brakeForce;
        coastDrag = profile.coastDrag;

        maxSpeedKPH = profile.maxSpeedKPH;
        maxReverseKPH = profile.maxReverseKPH;

        highSpeedAccelFactor = profile.highSpeedAccelFactor;
        launchBoostSpeed = profile.launchBoostSpeed;
        launchMinThrottle = profile.launchMinThrottle;

        steerAtLow = profile.steerAtLow;
        steerAtHigh = profile.steerAtHigh;
        steerResponse = profile.steerResponse;
        maxVisualSteerAngle = profile.maxVisualSteerAngle;

        lateralGrip = profile.lateralGrip;
        yawStability = profile.yawStability;
        extraDownforce = profile.extraDownforce;

        highSpeedGripAssist = profile.highSpeedGripAssist;
        lateralVelocityKill = profile.lateralVelocityKill;
        steeringSpeedLossReducer = profile.steeringSpeedLossReducer;
        rollStability = profile.rollStability;
        pitchStability = profile.pitchStability;

        driftSideGrip = profile.driftSideGrip;
        normalSideGrip = profile.normalSideGrip;
        driftYawAssist = profile.driftYawAssist;
        driftMinSpeedKmh = profile.driftMinSpeedKmh;

        airControl = profile.airControl;
        airDownforce = profile.airDownforce;

        nitroForwardForce = profile.nitroForwardForce;
        nitroAccelerationMultiplier = profile.nitroAccelerationMultiplier;
        nitroMaxSpeedBonus = profile.nitroMaxSpeedBonus;

        maxGear = profile.maxGear;

        if (profile.gearSpeedLimits != null && profile.gearSpeedLimits.Length > 0)
            gearSpeedLimits = (float[])profile.gearSpeedLimits.Clone();

        shiftUpBuffer = profile.shiftUpBuffer;
        shiftDownBuffer = profile.shiftDownBuffer;
        shiftCooldown = profile.shiftCooldown;
        shiftSpeedDrop = profile.shiftSpeedDrop;
        downshiftBufferKPH = profile.downshiftBufferKPH;

        maxRPM = profile.maxRPM;
        rpmSmoothing = profile.rpmSmoothing;
    }
    void Start()
    {
        ...
        if (inputController == null)
            inputController = gameObject.AddComponent<VehicleInputController>();

        inputController.UseExternalInput = useExternalInput;

        if (applyTuningProfileOnStart && tuningProfile != null)
        {
            if (applyUpgradeStateOnStart)
            {
                runtimeTuningProfile = VehicleTuningCalculator.CreateRuntimeProfile(
                    tuningProfile,
                    upgradeState
                );

                ApplyTuningProfile(runtimeTuningProfile);
            }
            else
            {
    ...
    public void SetExternalInput(float steer, float throttle)
    {
        ...
        if (inputController != null)
        {
            ...
            inputController.SetExternalInput(steer, throttle);
        }
    ...
    public void SetExternalHandbrake(bool value)
    {
        ...
        if (inputController != null)
            inputController.SetExternalHandbrake(value);
    }
    ...
    void ReadInput()
    {
        ...
        VehicleDriveInput driveInput = inputController.CurrentInput;

        moveInput = driveInput.MoveVector;
        steerInput = driveInput.steer;
        throttleInput = driveInput.throttle;
    }
    ...
    void ApplyDriftAssist(float steerInput)
    {
        ...
        bool handbrakePressed =
            ...
            inputController.CurrentInput.handbrake;
    ...
    public VehicleUpgradeState GetUpgradeState()
    {
        return upgradeState;
    }

    public void SetUpgradeState(VehicleUpgradeState newState, bool applyImmediately = true)
    {
        if (newState == null)
            return;

        upgradeState.engineLevel = newState.engineLevel;
        upgradeState.transmissionLevel = newState.transmissionLevel;
        upgradeState.nitroLevel = newState.nitroLevel;
        upgradeState.handlingLevel = newState.handlingLevel;
        upgradeState.suspensionLevel = newState.suspensionLevel;
        upgradeState.Clamp();
    ...
    public void ReapplyTuning()
    {
        ...
        if (runtimeTuningProfile != null)
            Destroy(runtimeTuningProfile);
...
#if UNITY_EDITOR
    [ContextMenu("Tuning/Copy Current Values To Assigned Profile")]
    private void CopyCurrentValuesToAssignedProfile()
    {
        if (tuningProfile == null)
        {
            Debug.LogWarning("[CarControll] No tuning profile assigned.");
            return;
        }

        tuningProfile.acceleration = acceleration;
        tuningProfile.reverseAcceleration = reverseAcceleration;
        tuningProfile.brakeForce = brakeForce;
        tuningProfile.coastDrag = coastDrag;

        tuningProfile.maxSpeedKPH = maxSpeedKPH;
        tuningProfile.maxReverseKPH = maxReverseKPH;

        tuningProfile.highSpeedAccelFactor = highSpeedAccelFactor;
        tuningProfile.launchBoostSpeed = launchBoostSpeed;
        tuningProfile.launchMinThrottle = launchMinThrottle;

        tuningProfile.steerAtLow = steerAtLow;
        tuningProfile.steerAtHigh = steerAtHigh;
        tuningProfile.steerResponse = steerResponse;
        tuningProfile.maxVisualSteerAngle = maxVisualSteerAngle;

        tuningProfile.lateralGrip = lateralGrip;
        tuningProfile.yawStability = yawStability;
        tuningProfile.extraDownforce = extraDownforce;

        tuningProfile.highSpeedGripAssist = highSpeedGripAssist;
        tuningProfile.lateralVelocityKill = lateralVelocityKill;
        tuningProfile.steeringSpeedLossReducer = steeringSpeedLossReducer;
        tuningProfile.rollStability = rollStability;
        tuningProfile.pitchStability = pitchStability;

        tuningProfile.driftSideGrip = driftSideGrip;
        tuningProfile.normalSideGrip = normalSideGrip;
        tuningProfile.driftYawAssist = driftYawAssist;
        tuningProfile.driftMinSpeedKmh = driftMinSpeedKmh;

        tuningProfile.airControl = airControl;
        tuningProfile.airDownforce = airDownforce;

        tuningProfile.nitroForwardForce = nitroForwardForce;
        tuningProfile.nitroAccelerationMultiplier = nitroAccelerationMultiplier;
        tuningProfile.nitroMaxSpeedBonus = nitroMaxSpeedBonus;

        tuningProfile.maxGear = maxGear;

        if (gearSpeedLimits != null)
            tuningProfile.gearSpeedLimits = (float[])gearSpeedLimits.Clone();

        tuningProfile.shiftUpBuffer = shiftUpBuffer;
        tuningProfile.shiftDownBuffer = shiftDownBuffer;
        tuningProfile.shiftCooldown = shiftCooldown;
        tuningProfile.shiftSpeedDrop = shiftSpeedDrop;
        tuningProfile.downshiftBufferKPH = downshiftBufferKPH;

        tuningProfile.maxRPM = maxRPM;
        tuningProfile.rpmSmoothing = rpmSmoothing;

        UnityEditor.EditorUtility.SetDirty(tuningProfile);
        UnityEditor.AssetDatabase.SaveAssets();

        Debug.Log($"[CarControll] Copied current values to tuning profile: {tuningProfile.name}");
    }
#endif

#if UNITY_EDITOR
    [ContextMenu("Tuning/Reapply Runtime Tuning")]
    private void EditorReapplyRuntimeTuning()
    {
        ReapplyTuning();
    }

    [ContextMenu("Tuning/Reset Upgrade State")]
    private void EditorResetUpgradeState()
    {
        upgradeState.engineLevel = 0;
        upgradeState.transmissionLevel = 0;
        upgradeState.nitroLevel = 0;
        upgradeState.handlingLevel = 0;
        upgradeState.suspensionLevel = 0;
